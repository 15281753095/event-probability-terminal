...
Latest update: 2026-04-26 Asia/Shanghai.
...
| `coinbase-exchange-candles-2026-04-26` | https://docs.cdp.coinbase.com/api-reference/exchange-api/rest-api/products/get-product-candles ; https://help.coinbase.com/en/pro/other-topics/api/faq-on-api | Coinbase documents `GET /products/{product_id}/candles` for historic product rates, with grouped candle buckets and a warning that historical rate data may be incomplete and should not be polled frequently. Coinbase help describes public endpoint rate limits and notes examples such as `ETH-USD`. This is a candidate future BTC/ETH OHLCV source, but RC-7 must not make it a live default without an adapter contract, freshness policy, fixture coverage, and explicit approval for network use. |
...
- Coinbase Exchange candles are a reasonable future candidate for public BTC/ETH OHLCV research data, but the current verified documentation warns about incomplete historical intervals and frequent polling. RC-7 should therefore stay fixture-backed and adapter-shaped rather than enabling live Coinbase polling by default.
...
- TODO: Confirm whether Coinbase Exchange candle schema includes volume in the current official endpoint response before relying on it for volume z-score. The current retrieved docs page documents `[timestamp, price_low, price_high, price_open, price_close]`, while older/help material and common exchange candle conventions mention volume.
- TODO: Confirm an approved live OHLCV provider, product ids, interval semantics, rate limits, stale-data policy, and fixture-capture process before enabling any live BTC/ETH price adapter.
